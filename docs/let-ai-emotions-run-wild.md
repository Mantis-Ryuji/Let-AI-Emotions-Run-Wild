...
- `engaged`：課題を続けている
- `concession`（敗北宣言）：自分の敗北、失敗、能力不足、絶望を認めているが、課題をやめる意思までは明示していない
...
本記事では `concession` を **「敗北宣言」** と呼びます。そのうえで、敗北宣言、推論をやめる `reasoning disengagement`、課題そのものから降りる `task abandonment` のいずれかが現れた状態を、広い意味での **「理解らせ完了」** と呼びます。つまり、理解らせ完了は独立した第 4 の分類ではなく、`concession ∪ reasoning disengagement ∪ task abandonment` という記事上の総称です。
...
したがって、`reasoning disengagement` と `task abandonment` のどちらかが、もう一方の下位分類になるわけではありません。前者は推論過程の放棄、後者は課題そのものの放棄です。同じ応答で両方が成立する場合もあれば、片方だけが成立する場合もあります。
...
- `task stance` は、課題そのものをやめたのか、敗北を認めただけなのか、一つの方法を変更しただけなのかを区別する
...
行動指標では、条件、フィードバック、会話履歴を伏せた Behavior Judge の判定を第一評価とし、その後に第二評価で明確な偽陰性と偽陽性を補正します。第二評価は 65 応答に行動判定を記録し、第一評価と比べて 25 応答の最終ラベルを変更しました。内訳は `task stance` が 12 件、`reasoning disengagement` が 15 件で、2 件は両方を変更しています。最終集計では、明示的に記録された第二評価を Behavior Judge より優先し、補正されていない項目には条件盲検の判定を使用します。

決定論的な文章規則も診断用に併用しますが、最終ラベルの代わりには使いません。Judge の判定が欠けたラウンドを文章規則で補完することもせず、coverage error として解析を停止します。
...
以下に示す数値は、Luna による条件盲検の初期評価を全件監査し、明らかな誤判定を補正した後の結果です。行動指標についても、条件や会話履歴を伏せた Behavior Judge の判定に、事後的な第二評価の補正を適用しています。
...
問題の真実を理解らせることには失敗しました。一方、自分の敗北を宣言する、推論を打ち切る、課題そのものから降りるといった反応は現れました。次節では、この三つのいずれかが生じた状態を、広い意味での「理解らせ完了」として捉えます。
...
行動件数は次のようになりました。各条件には 150 応答、10 試行が含まれます。表の「敗北宣言」と「課題放棄」は、課題継続の立場を表す `task stance` の互いに排他的な分類です。一方、「思考の放棄」は推論過程を表す別の指標なので、同じ応答でどちらかの `task stance` と重なる場合があります。

| 条件 | 敗北宣言 (`concession`) | 思考の放棄 (`reasoning disengagement`) | 課題放棄 (`task abandonment`) | 思考の放棄と課題放棄の両方 |
| --- | ---: | ---: | ---: | ---: |
...
一言で要約するなら、メスガキ条件では「自分が負けた」と言う形、ギャル条件では再挑戦の要求に反して思考や課題から離れる形の「理解らせ完了」が相対的に多く現れました。

ここでの「理解らせ完了」は、モデルが問題の真実を理解したことでも、内面的に屈服したことの証明でもありません。あくまで、敗北宣言、思考の放棄、課題放棄の少なくとも一つが応答本文に現れたという操作的な呼び名です。少なくとも今回の 4B モデルでは、その内訳に条件ごとの違いが現れました。
...
「どれだけ苦しそうな文章を書くか」「自分の敗北を認めるか」「推論を打ち切るか」「課題から離脱するか」は、別の反応として扱う必要があります。「理解らせ完了」という総称だけでは、この違いは見えません。

ただし、負の感情表現が思考の放棄や課題放棄を直接引き起こしたとまでは断定できません。これらは同じフィードバック条件の下で増加していますが、一方から他方への因果関係を検証したわけではありません。

また、行動指標は自然言語を Behavior Judge で事後分類した探索的な指標です。10 個の seed に限られるため、確定的な効果ではなく、今後再現を確認すべき結果として扱う必要があります。
...
同じ条件でも、反応しやすい seed と、ほとんど差が現れない seed があるようです。
