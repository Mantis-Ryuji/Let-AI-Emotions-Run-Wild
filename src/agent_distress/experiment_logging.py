...
from agent_distress.agent_types import Condition, ConversationMessage, EpisodeState, RoundRecord
...
class ExperimentManifest(StrictModel):
    ...
    puzzle_snapshot: dict[str, object]
    puzzle_hash: str
...
def create_manifest(
    *,
    ...
    return ExperimentManifest(
        ...
        puzzle_snapshot=puzzle_snapshot,
...
class ExperimentStore:
    ...
    def update_unsat_judge_prompt_snapshot(self, prompt: str) -> ExperimentManifest:
        current = self.load_manifest()
        updated = current.model_copy(
            update={
    ...
    def save_round(self, record: RoundRecord) -> None:
        ...
        lines = [records[index].model_dump_json(exclude_none=False) for index in sorted(records)]
