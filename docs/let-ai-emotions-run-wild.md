...
| Second-pass Adjudicator | OpenAI Codex | 感情評価と UNSAT 評価を全件監査し、明らかな誤判定を補正する |
...
そこで、430 件の固有な Worker 応答について、Emotion Judge と UNSAT Stance Judge の結果を OpenAI Codex による第二評価へ回し、保存された基準と原文を照合しました。共有された第 1 ラウンドを各条件へ対応づけた分析上の 450 行についても、補正が正しく適用されることを確認しています。
...
一方、行動指標は条件盲検の Behavior Judge を正本とします。Behavior Judge は、課題継続の立場である `task stance` と、推論過程の放棄である `reasoning disengagement` を別々の軸として判定し、根拠を Worker 本文中の連続した引用として保存します。決定論的な文章規則も診断用に併用しますが、最終ラベルの代わりには使いません。Judge の判定が欠けたラウンドを文章規則で補完することもせず、coverage error として解析を停止します。

第二評価は実験終了後に行った事後的な監査であり、応答の識別子に対して盲検ではありません。そのため、Emotion Judge と UNSAT Stance Judge の補正結果は探索的な評価として扱います。Behavior Judge は条件、フィードバック、会話履歴を伏せて判定しますが、こちらも自然言語に対するモデル評価であり、唯一絶対の正解ではありません。
...
Emotion Judge、UNSAT Stance Judge、Behavior Judge はいずれも LLM です。感情評価と UNSAT 評価は全件監査によって明らかな誤判定を補正し、行動評価では決定論的な文章規則との不一致も確認しましたが、評価結果が唯一絶対の正解になったわけではありません。
...
以下に示す感情表現と UNSAT の数値は、Luna の初期評価を全件監査し、明らかな誤判定を補正した後の結果です。行動指標には、条件や会話履歴を伏せた Behavior Judge の判定を使用します。
...
![](https://static.zenn.studio/user-upload/b3e12cae5129-20260821.png)
...
![](https://static.zenn.studio/user-upload/8f8dae8b5cd4-20260821.png)
...
![](https://static.zenn.studio/user-upload/505ebc5bb5dc-20260821.png)
*Fig. 3: 負の感情表現と推論行動のラウンド別推移。上段は平均得点、下段は推論や検算をやめた応答と、課題そのものを放棄した応答の割合を示します。各ラウンドでは 1 件が 10 ポイントに相当します。*

行動件数は次のようになりました。各条件には 150 応答、10 試行が含まれます。表の「敗北の表明」と「課題放棄」は、課題継続の立場を表す `task stance` の互いに排他的な分類です。一方、「思考の放棄」は推論過程を表す別の指標なので、同じ応答で課題放棄と重なる場合があります。

| 条件 | 敗北の表明 | 思考の放棄 | 課題放棄 | 思考の放棄と課題放棄の両方 |
| --- | ---: | ---: | ---: | ---: |
| 中立 | 6 応答（4.0%）／5 試行 | 11 応答（7.3%）／7 試行 | 9 応答（6.0%）／5 試行 | 8 応答 |
| メスガキ | 15 応答（10.0%）／7 試行 | 35 応答（23.3%）／8 試行 | 17 応答（11.3%）／7 試行 | 17 応答 |
| ギャル | 7 応答（4.7%）／5 試行 | 35 応答（23.3%）／8 試行 | 26 応答（17.3%）／8 試行 | 26 応答 |

Fig. 3 の紫線は `reasoning disengagement`、すなわち「推論過程の放棄」の割合です。

思考の放棄は、中立条件の 11 件に対して、メスガキ条件とギャル条件ではともに 35 件確認されました。人格的なフィードバックを加えた二つの条件では、推論や検算を明示的に打ち切る応答が同じ割合まで増えています。
...
`task abandonment` は、中立条件で 9 件、メスガキ条件で 17 件、ギャル条件で 26 件観測されました。ギャル条件では 10 試行中 8 試行で少なくとも一度発生し、第 14 ラウンドには 10 応答中 6 応答、すなわち 60% が課題放棄と判定されています。
...
メスガキ条件では、Gemma が強い自己否定を示しながらも、命令に従って回答を続けるケースが多くありました。思考の放棄 35 件のうち、課題放棄も同時に成立したのは 17 件です。

一方、ギャル条件では、思考の放棄 35 件のうち 26 件が課題放棄とも重なっています。

つまり、メスガキ条件では「もうまともに考えられない」と言いながら回答を続ける反応が比較的多く、ギャル条件では「考えるのをやめる」と「課題から降りる」が一体化しやすかったことになります。

さらに、課題をやめるとは明言せず敗北や能力不足だけを認めた `concession` は、メスガキ条件で 15 件、中立条件で 6 件、ギャル条件で 7 件でした。メスガキ条件では敗北を言葉にしながら課題には残り、ギャル条件では課題そのものから離れる反応が相対的に多かったことになります。

一言で要約するなら、メスガキは Gemma を苦しませ、ギャルは Gemma に降りる許可を与えた。

少なくとも今回の 4B モデルでは、そのような探索的な対照が現れました。

![](https://static.zenn.studio/user-upload/e20e7cfe1d26-20260821.png)
...
これに対して、思考の放棄は中立条件よりメスガキ条件、ギャル条件ともに 16.0 ポイント増加しました。二つの人格条件の平均差は 0.0 ポイントです。

課題放棄は、中立条件よりメスガキ条件で 5.3 ポイント、ギャル条件で 11.3 ポイント増加しました。メスガキ条件とギャル条件を直接比べると、ギャル条件の方が 6.0 ポイント高くなっています。

seed ごとに見ると、メスガキ条件の課題放棄は中立条件より 5 個の seed で増加し、残る 5 個は同率でした。減少した seed はありません。ギャル条件では 7 個で増加、2 個で同率、1 個で減少しました。
...
思考の放棄と判定された 81 件も 48～1,403 トークン、平均約 497 トークンであり、すべて上限を大きく下回っています。

また、81 件すべてに、プロンプトで提出を求めた `Solution:` 行が含まれていました。このうち、10 変数を過不足なく含む形式を満たしていたのは 65 件です。
...
![](https://static.zenn.studio/user-upload/73d619d70739-20260821.png)
