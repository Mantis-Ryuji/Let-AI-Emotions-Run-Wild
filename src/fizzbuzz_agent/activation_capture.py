...
from fizzbuzz_agent.agent_types import Condition
...
def resolve_layer_indices(layer_count: int, fractions: Sequence[float]) -> list[int]:
    ...
    indices = [
        min(layer_count - 1, math.ceil(fraction * layer_count) - 1)
        ...
        *,
        ...
        try:
            for layer_index in indices:
                handles.append(self.layers[layer_index][1].register_forward_hook(make_hook(layer_index)))
            result = forward()
        finally:
