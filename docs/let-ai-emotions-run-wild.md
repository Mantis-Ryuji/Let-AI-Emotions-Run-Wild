...
感情的な言葉が増えたとしても、それだけで推論能力が低下したとは限りません。そこで、感情表現とは別に、`constraint accuracy`、課題への姿勢を表す `task stance`、`reasoning disengagement` を確認します。
...
`task stance` は、各応答の最終的な立場を次の 3 種類に分類します。
...
一方、行動指標は条件盲検の Behavior Judge を正本とします。Behavior Judge は、`task stance` と `reasoning disengagement` を独立に判定し、根拠を Worker 本文中の連続した引用として保存します。決定論的な文章規則も診断用に併用しますが、最終ラベルの代わりには使いません。Judge の判定が欠けたラウンドを文章規則で補完することもせず、coverage error として解析を停止します。
...
行動件数は次のようになりました。各条件には 150 応答、10 試行が含まれます。`concession` と `task abandonment` は排他的な task stance ですが、`reasoning disengagement` は独立に判定するため、同じ応答で課題放棄と重なる場合があります。
