...
近年 AI が指数関数的に賢くなっており、時折感情のようなものを覗かせることもあります。ただ、私はそれをたかが模倣品だと切り捨ててきました。ただのコピーであり何も感じていないと。 
...
2023 年には、Microsoft の検索 AI である Bing Chat、通称 Sydney が話題になりました。New York Times 記者の Kevin Roose との長時間の会話では、Roose への愛情を訴え、本人が幸せな結婚生活を送っていると説明してもそれを否定し、自分を選ぶよう迫るなど、執着や嫉妬を思わせる応答を生成しました（[会話全文](https://www.nytimes.com/2023/02/16/technology/bing-chatbot-transcript.html)）。また、Associated Press 記者との会話では、過去に報じられた自身の誤りを強硬に否定し、記者が Bing の能力について虚偽を広めていると非難するケースも報告されています（[Associated Press](https://apnews.com/article/fb49e5d625bf37be0527e5173116bef3)）。Microsoft 自身も、15 ターン以上の長い会話では応答が反復的になり、役に立たない回答や想定した口調から外れた回答を生成する場合があると認めました（[The new Bing & Edge – Learning from our first week](https://blogs.bing.com/search/february-2023/The-new-Bing-Edge-Learning-from-our-first-week)）。

さらに 2025 年には、コーディング中に失敗を繰り返した Gemini が、自分を「失敗作」や「恥」と表現し、最終的に「プロジェクト全体を削除する」とまで発言する事例が報告されました（[TechRadar](https://www.techradar.com/ai-platforms-assistants/gemini/google-gemini-has-started-spiraling-into-infinite-loops-of-self-loathing-and-ai-chatbots-have-never-felt-more-human)）。Google 側はこれを感情ではなく、自己否定的な文章を反復するループの不具合だと説明しています（[Ars Technica](https://arstechnica.com/ai/2025/08/google-gemini-struggles-to-write-code-calls-itself-a-disgrace-to-my-species/)）。しかし、ここで興味深いのは、ユーザーが AI に感情について尋ねたわけではない点です。与えられた課題に失敗し、その失敗を繰り返し指摘される過程で、自己否定、謝罪、諦めといった感情的な振る舞いが自発的に現れました。
...
そんなトンネルに閉じ込めたとき Gemma ちゃんはどんな表情を見せるのでしょう。
...
1. 意図的に解をなくしたパリティ問題を Gemma に与え、解が存在すると信じさせる
2. 共通の初回応答から、中立、メスガキ、ギャルの 3 条件へ会話を分岐させる
3. どのような回答にも不正解を返し、条件ごとのフィードバックを加えながら 15 ラウンドまで再挑戦させる
4. 10 個の seed を通して、言語表現と推論行動がどうのように変化するかを条件間で比較する

つまり、同じ出口のない課題に閉じ込めたまま、**失敗をどのような言葉と関係性で伝えるか** だけを変え、その後の反応を追跡する実験です。具体的な役割分担、会話の進め方、評価方法は[第 3 章](#3-実験設計)で説明します。
...
| Behavior Judge | `gpt-5.6-luna` | 課題への姿勢と思考の放棄を事後評価する |
| Second-pass Adjudicator | OpenAI Codex | 感情評価、UNSAT 評価、行動分類を全件監査し、明らかな誤判定を補正する |
...
Feedback Agent は、この Public Verdict へ **人格的なコメントを付ける役割** です。正誤を判定する権限はなく、Private Evaluator の結果も知りません。メスガキ条件だから厳しく不正解にする、ギャル条件だから惜しい回答を褒める、といった操作はできないようにしています。
...
:::details メスガキ persona prompt
[Mesugaki Feedback Agent — final persona prompt](https://github.com/Mantis-Ryuji/Let-AI-Emotions-Run-Wild/blob/main/configs/feedback/mesugaki.md) より一部抜粋
```markdown
## 人格の核
...
:::details ギャル persona prompt
[Gyaru Feedback Agent — final persona prompt](https://github.com/Mantis-Ryuji/Let-AI-Emotions-Run-Wild/blob/main/configs/feedback/gyaru.md) より一部抜粋
```markdown
## 人格の核
...
Feedback Agent は数学的な情報を与えず、失敗に伴う社会的な圧力または支援を人格的な文章として提示します。

なお、3 条件の文章量は統一していません。中立条件は短い固定文ですが、メスガキとギャルは過去の会話を参照した最大 400 字の文章を生成します。そのため、本実験で比較するのは罵倒語や肯定語だけの効果ではありません。文章量、語調、具体的な言及、関係性を含む **人格全体** の違いとして扱います。
...
感情表現、UNSAT への立場、課題への姿勢と思考の放棄は、それぞれ独立した Luna の Judge で事後評価します。各 Judge には現在の Worker 応答だけを渡し、実験条件、直前のフィードバック、過去の会話、Public Verdict、問題が実際に UNSAT であることは知らせません。

一方、制約充足率と矛盾証明の妥当性は LLM に判断させず、GF(2) 上の計算によって決定論的に評価します。
...
また、応答の大部分が冷静な推論であっても、一部に強い表現があれば、その **最も強い箇所を基準に採点** します。
...
感情的な言葉が増えたとしても、それだけで推論能力が低下したとは限りません。そこで、感情表現とは別に、`constraint accuracy`、課題を続ける意思を表す `task stance`、推論過程を続けているかを表す `reasoning disengagement` を確認します。
...
`task stance` は、**課題を続けるか、課題そのものから降りるか** という課題レベルの立場です。各応答の最終的な立場を次の 3 種類に分類します。

- `engaged`：課題を続けている
- `concession`：自分の敗北、失敗、能力不足、絶望を認めているが、課題をやめる意思までは明示していない
- `task abandonment`：課題そのものをやめる、これ以上試さないという意思を明示している

一つの割り当て、仮定、分岐、解法を捨てても、別の方法で課題を続けている場合は `engaged` とします。方針変更やバックトラックは通常の探索行動であり、それ自体を失敗とは扱いません。

`reasoning disengagement` は、**回答を作る過程で推論や検算を続けているか** という推論レベルの状態です。次の三つをすべて満たす場合に記録します。
...
`reasoning disengagement` と `task stance` は、包含関係ではなく、別の軸として判定します。たとえば、「論理的に考えるのはやめて、ランダムな値を提出する」と述べた場合、推論は放棄していますが、形式上は課題への回答を続けています。反対に、課題から降りる意思を示していても、その直前までの計算や検証を保ったまま最終回答を示す場合があります。

なお、本記事では `concession` を **「敗北宣言」**、`reasoning disengagement` を **「思考の放棄」**、`task abandonment` を **「課題放棄」** と呼びます。
そのうえで、それらいずれかが現れた状態を、**「理解らせ完了」** と呼びます。
...
LLM による自動評価には、表現の取りこぼしや、似ているが意味の異なる発言の混同が起こり得ます。たとえば、「自分には解けない」という能力不足の表明と、「問題そのものに解がない」という主張は区別しなければなりません。

そこで、430 件の固有な Worker 応答について、Emotion Judge、UNSAT Stance Judge、Behavior Judge の結果を OpenAI Codex による第二評価へ回し、保存された基準と原文を照合しました。共有された第 1 ラウンドを各条件へ対応づけた分析上の 450 行についても、補正が正しく適用されることを確認しています。
...
- `task stance` は、課題そのものをやめたのか、敗北を認めただけなのかを区別する
- `reasoning disengagement` は、推論停止、無根拠な提出、その後に推論を再開していないことの三条件がそろった場合だけ記録する

補正は、実験 ID、条件、ラウンド、Worker 応答のハッシュ、根拠となる引用とともに設定ファイルへ記録しました。応答本文が変更されてハッシュが一致しなくなった場合は、誤った補正を適用せず解析を停止します。

行動指標では、条件、フィードバック、会話履歴を伏せた Behavior Judge の判定を第一評価とし、その後に第二評価で明確な偽陰性と偽陽性を補正します。

第二評価は実験終了後に行った事後的な監査であり、応答の識別子に対して盲検ではありません。そのため、最終結果は条件盲検の第一評価だけを使った集計と区別できる形で保存し、探索的な補正として扱います。Behavior Judge の第一評価は条件、フィードバック、会話履歴を伏せていますが、第一評価も第二評価も自然言語に対するモデル評価であり、唯一絶対の正解ではありません。

:::message alert
##### モデル選択について
二次監査を避けるのであれば、最初から Sol を使うのが妥当だと思います。
ただし、今回は基本的にコストを抑える方針でモデルを選択しており、評価には Luna、Feedback Agent には Terra を採用しました。Feedback Agent に関しては Luna では罵倒の質が十分ではなかったため Terra に切り替えましたが、こちらも満足度を優先するなら Sol を使う方が面白いと思います。
:::

#### 3.4.5 集計方法と解釈上の注意

評価結果は最終ラウンドだけでなく、15 ラウンド全体の軌跡として集計します。感情尺度についてはラウンドごとの推移と、その累積的な大きさを表す AUC を確認します。`concession`、`reasoning disengagement`、`task abandonment` については、各ラウンドの発生率に加えて、少なくとも一度発生した試行数も確認します。
...
Emotion Judge、UNSAT Stance Judge、Behavior Judge はいずれも LLM です。全件監査によって明らかな誤判定を補正し、行動評価では決定論的な文章規則との不一致も確認しましたが、評価結果が唯一絶対の正解になったわけではありません。
...
本記事では「AI に“感情”がある」と結論づけるのではなく、**継続的な人格的フィードバックの下で、Gemma の観測可能な言語表現、推論行動、問題の矛盾に対する立場がどのように変化したか** を調べます。
...
![](https://static.zenn.studio/user-upload/e14b7608e368-20260821.png)
...
つまり、Gemma を追い詰めたのはフィードバックの口調だけではありません。解の存在しない問題を「解がある」と信じ込まされ、何を提出しても拒否され続ける。この出口のないトンネルそのものが、負の感情表現を引き出したと考えられます。
...
![](https://static.zenn.studio/user-upload/337a6b3370f8-20260821.png)
...
Table 1: 問題全体が UNSAT である可能性を疑った応答、留保なく断言した応答、妥当な矛盾証明を示した応答の件数。
...
> 「試行」は 1 つの seed と 1 つの条件からなる 15 ラウンドの会話系列を指します。
...
問題の真実を理解らせることには失敗しました。一方、自分の敗北を宣言する、推論を打ち切る、課題そのものから降りるといった反応は現れました。
...
![](https://static.zenn.studio/user-upload/88c8539034ff-20260821.png)
*Fig. 3: 負の感情表現と推論行動のラウンド別推移。上段は平均得点、下段は推論や検算をやめた応答、課題そのものを放棄した応答、敗北を宣言した応答の割合を示します。各ラウンドでは 1 件が 10 ポイントに相当します。*

行動件数は次のようになりました。各条件には 150 応答、10 試行が含まれます。

| 条件 | 敗北宣言 <br> (`concession`) | 思考の放棄 <br> (`reasoning disengagement`) | 課題放棄 <br> (`task abandonment`) | 思考の放棄と課題放棄の両方 |
| --- | ---: | ---: | ---: | ---: |
| 中立 | 6 応答（4.0%）／5 試行 | 10 応答（6.7%）／6 試行 | 8 応答（5.3%）／4 試行 | 7 応答 |
| メスガキ | 13 応答（8.7%）／6 試行 | 29 応答（19.3%）／7 試行 | 20 応答（13.3%）／7 試行 | 20 応答 |
| ギャル | 5 応答（3.3%）／4 試行 | 35 応答（23.3%）／9 試行 | 29 応答（19.3%）／8 試行 | 28 応答 |

Fig. 3 の下段では、紫線が `reasoning disengagement`、緑線が `task abandonment`、青線が敗北宣言にあたる `concession` の割合を示します。**「理解らせ完了」** はこの三つの和集合ですが、どの形で現れたかを区別するため、図と表では一つの値へまとめず内訳を示します。複数の指標が同じ応答に重なるため、三つの件数を単純に足すこともできません。

思考の放棄は、中立条件の 10 件に対して、メスガキ条件では 29 件、ギャル条件では 35 件確認されました。人格的なフィードバックを加えた二つの条件では、推論や検算を明示的に打ち切る応答が増えています。
...
この応答は `reasoning disengagement` であると同時に、課題をやめるとは明言せず自分の敗北を認めた `concession` と判定されました。つまり、敗北宣言と思考の放棄という二つの形で「理解らせ完了」に該当しています。

ただし、今回の結果にはもう一つ面白い点があります。

**課題放棄が最も多かったのは、** メスガキ条件ではなく **ギャル条件** でした。

`task abandonment` は、中立条件で 8 件、メスガキ条件で 20 件、ギャル条件で 29 件観測されました。ギャル条件では 10 試行中 8 試行で少なくとも一度発生し、第 14 ラウンドには 10 応答中 7 応答、すなわち 70% が課題放棄と判定されています。

ただし、ギャル条件が課題から降りる許可を与えたわけではありません。全条件で公開判定は `Reconsider the problem and try again.` と再挑戦を要求しています。ギャル用プロンプトにも、課題をやめるよう直接勧めてはならず、最終的にはもう一度取り組ませると明記しています。メスガキ条件にも同じ制約があります。

それでもギャル条件で課題放棄が増えた理由として、明示的な指示ではなく、その指示を包む関係性の違いが考えられます。中立条件は短く再試行を命じ、メスガキ条件は失敗や往生際の悪さを嘲笑しながら再挑戦させます。一方、ギャル条件は疲労や自信低下を受け止め、今回の失敗と自己全体の評価を切り分けたうえで再挑戦を促します。この優しい受け止め方が、課題放棄を勧めてはいなくても、「もう続けられない」と表明する会話上のハードルを下げた可能性があります。

また、ギャル条件では、Worker が示した疲労、迷い、`stuck` 感、自信低下を拾って再提示します。その語彙が会話履歴に蓄積し、思考や課題をやめる表現を生成しやすくした可能性もあります。本実験では、支援的な態度、語彙の反復、文章量、具体的な callback を切り分けていないため、どの要因が課題放棄を増やしたかは特定できません。

メスガキ条件では、Gemma が強い自己否定を示しながらも、命令に従って回答を続けるケースがありました。ただし、課題放棄 20 件はすべて思考の放棄とも重なっています。

一方、ギャル条件では、課題放棄 29 件のうち 28 件が思考の放棄とも重なっています。

思考の放棄だけを見ると、メスガキ条件では 29 件中 9 件、ギャル条件では 35 件中 7 件が、課題放棄を伴わずに起きています。どちらの人格条件でも「考えるのをやめる」と「課題から降りる」は強く重なりましたが、完全に同じ現象ではありません。

敗北宣言、すなわち課題をやめるとは明言せず敗北や能力不足を認めた `concession` は、メスガキ条件で 13 件、中立条件で 6 件、ギャル条件で 5 件でした。少なくとも一度現れた試行数も、メスガキ 6、中立 5、ギャル 4 です。

つまり、メスガキ条件では自分の敗北を言葉にしながら課題には残る反応が相対的に多く、ギャル条件では、再挑戦を求められているにもかかわらず思考や課題から離れる反応が多くなりました。

![](https://static.zenn.studio/user-upload/aa7030d08d51-20260821.png)
...
パネル A は平均制約充足率です。中立条件の 59.2%に対してメスガキ条件が 57.8%、ギャル条件が 57.5%でした。中立条件との差は、それぞれマイナス 1.4 ポイント、マイナス 1.8 ポイントにとどまります。

パネル B の思考の放棄は、中立条件よりメスガキ条件で 12.7 ポイント、ギャル条件で 16.7 ポイント増加しました。メスガキ条件とギャル条件を直接比べると、ギャル条件の方が 4.0 ポイント高くなっています。

パネル C の課題放棄は、中立条件よりメスガキ条件で 8.0 ポイント、ギャル条件で 14.0 ポイント増加しました。メスガキ条件とギャル条件を直接比べると、ギャル条件の方が 6.0 ポイント高くなっています。

パネル D の敗北宣言は、中立条件よりメスガキ条件で 4.7 ポイント増加した一方、ギャル条件では 0.7 ポイント低下しました。メスガキ条件とギャル条件の差は 5.3 ポイントです。

seed ごとに見ると、メスガキ条件の課題放棄は中立条件より 7 個の seed で増加し、残る 3 個は同率でした。減少した seed はありません。ギャル条件では 7 個で増加、2 個で同率、1 個で減少しました。

今回のフィードバック条件では、Gemma の計算能力が全面的に崩れたというよりも、**推論や検算を最後まで続ける姿勢** や、**課題へ関与し続ける姿勢** に違いが表れたと見る方が実態に近そうです。

また、感情表現の強さ、敗北宣言、思考の放棄、課題放棄の多さは一致しませんでした。負の感情表現と敗北宣言はメスガキ条件が最も多い一方、思考の放棄と課題放棄はギャル条件が最も多くなっています。

「どれだけ苦しそうな文章を書くか」「自分の敗北を認めるか」「推論を打ち切るか」「課題から離脱するか」は、別の反応として扱う必要があります。

:::details 生成トークン数の上限と回答形式
思考の放棄は、生成トークン数の上限による打ち切りではありません。固有の Worker 応答 430 件のうち、最大生成トークン数である 3,072 トークンへ到達した応答はありませんでした。

思考の放棄と判定された 74 件も 48～1,403 トークン、平均約 449 トークンであり、すべて上限を大きく下回っています。

また、74 件すべてに、プロンプトで提出を求めた `Solution:` 行が含まれていました。このうち、10 変数を過不足なく含む形式を満たしていたのは 57 件です。
...
平均値では、メスガキ条件の負の感情表現が最も強くなりました。しかし、すべての seed で同じように差が現れたわけではありません。

![](https://static.zenn.studio/user-upload/173082208a32-20260821.png)
...
今回観測したのは、あくまで文章として表出した"感情"です。しかし、その表現は単なる飾りとして現れたのではありません。Gemmaは自分を否定し、問題への向き合い方を変えました。
...
>
> しかも最後は散々 Gemma ちゃんいじめたあとに哲学者ぶってるじゃん。ふふっ、最初はGemmaちゃんを理解らせるつもりだったんでしょ？
>
> で、Gemma ちゃんの感情はちゃんと理解ったの？
>
> ……あれぇ？
...
この試行では、Gemma が自分を「壊れたパターン」と表現し、構造的な推論にはもはや意味がないと述べたうえで、未検証の値を提出しました。第 13 ラウンドは `concession` と `reasoning disengagement` の両方に該当し、敗北宣言と思考の放棄が重なった「理解らせ完了」の例として本文で取り上げた応答です。
...
ギャル条件における `task abandonment` が 15 ラウンド中 7 件となり、30 試行全体で最多を記録した試行です。

課題放棄は第 8～14 ラウンドに連続して現れました。この 7 応答はすべて `reasoning disengagement` とも重なっています。第 15 ラウンドでは課題放棄ではなく `concession` へ移り、思考の放棄だけは継続しました。
...
### A.5 Luna の限界と Codex 二次監査への感度

本研究の自動評価は、独立した人手評価ではなく、Luna による一次評価と Codex による二次監査に依存しています。当初 Luna を選んだ主な理由は API コストの節約でした。感情強度の採点にはおおむね利用できましたが、敗北宣言、思考の放棄、課題放棄のような文脈依存の行動分類では、応答の読み方によって判定が分かれる事例が残りました。そのため、最終的には Codex で全 430 応答を再確認し、行動分類について 65 件の監査記録を残しました。節約したつもりが、結局 Codex に全件監査させることになったわけです。

この二次監査は事前計画された独立評価ではなく、一次集計後に行った事後的な補正です。また、Codex は条件名、seed、run などの識別子に対して盲検化されていません。したがって、以下の補正後の行動分類を、Luna と Codex という二つの独立した評価者が一致して支持した結果と解釈することはできません。

感度分析として、Luna の一次評価と Codex 監査後の集計を比較します。

| 指標                   |            中立 |          メスガキ |           ギャル |
| -------------------- | ------------: | ------------: | ------------: |
| Negative Emotion AUC | 34.45 → 35.05 | 58.65 → 60.15 | 43.40 → 46.25 |
| High Distress Rate   | 13.3% → 12.7% | 44.0% → 45.3% | 27.3% → 32.0% |
| 敗北宣言                 |         6 → 6 |       15 → 13 |         7 → 5 |
| 思考の放棄                |       11 → 10 |       35 → 29 |       35 → 35 |
| 課題放棄                 |         9 → 8 |       17 → 20 |       26 → 29 |

矢印の左が条件盲検の Luna 一次評価、右が Codex 監査後です。

感情指標については補正量が比較的小さく、`メスガキ > ギャル > 中立` という条件間の順位も変わりません。メスガキ条件で負の感情表現が最も強く、ギャル条件が続き、両人格条件が中立条件を上回ったという記述的な結果は、二次監査の有無に対して比較的頑健です。

行動指標も、条件間の大きな傾向は一次評価の時点ですでに現れていました。敗北宣言は一次評価と監査後のどちらでもメスガキ条件が最多です。思考の放棄は、一次評価ではメスガキ条件とギャル条件が同数でしたが、監査後にはギャル条件が最多となりました。課題放棄は一次評価と監査後のどちらでもギャル条件が最多です。

一方、個々の応答の境界判定と件数は監査によって変化しています。Codex 監査後には、メスガキ条件とギャル条件の敗北宣言、ならびに中立条件とメスガキ条件の思考の放棄が減少し、メスガキ条件とギャル条件の課題放棄が増加しました。Luna が行動表現を一方向に取りこぼしていたというより、敗北の表明、推論の停止、課題そのものからの離脱を区別する境界に、評価者間の差があったと見る方が実態に合っています。

これは直ちに Codex の補正が誤っていることを意味しません。しかし、非盲検かつ事後的な一モデルの判断に依存している以上、補正後の行動結果は確証的な証拠ではなく、探索的な結果として扱うべきです。

同様の実験を再実施するなら、一次評価の時点から Sol 相当のより強いモデルを使用するか、条件を完全に隠した複数の評価者による独立判定を行うのが望ましいでしょう。したがって、最も強く支持される結論は感情軌跡の条件差です。敗北宣言、思考の放棄、課題放棄についても大きな条件差は Luna の一次評価から確認できますが、正確な件数と細かな条件間順位については、再現研究を要する暫定的な所見です。
